...
- release decision as the package-candidate readiness and holdout or release-readiness layer
...
- grant final release-ready posture
...
`release_ready` means eligible for downstream repo-sync, freeze, version, and release work. It does not mean already published.
...
- holdout posture
- internal-only versus release-ready posture
...
The package candidate may enter downstream repo-sync, freeze, version, and release workflow.

This is the workflow's publish-ready outcome, but publication itself still happens only in the later release workflow from `#890`.
...
7. hand `release_ready`, `holdout`, and `approved_internal_only` outcomes to the downstream repo-sync and release workflow without coupling review directly to publication
