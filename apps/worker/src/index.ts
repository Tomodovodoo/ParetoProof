import { runProblem9AuthPreflightCli } from "./lib/problem9-auth-cli.js";
...
if (!command) {
  console.error(
    [
      ...
      "  tsx src/index.ts preflight-problem9-auth --auth-mode <mode> [--expect-codex-home <directory>]",
...
} else if (command === "preflight-problem9-auth") {
  void runProblem9AuthPreflightCli(args).catch((error: unknown) => {
    const message = error instanceof Error ? error.message : String(error);
    console.error(message);
    process.exitCode = 1;
  });
