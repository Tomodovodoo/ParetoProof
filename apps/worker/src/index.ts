...
const showWorkerUsage = () => {
  console.error(
    [
      ...
      "  tsx src/index.ts run-worker-claim-loop [--once] [--auth-mode machine_api_key|machine_oauth] [--provider-family <family>] [--provider-model <model>] [--worker-id <id>] [--worker-pool <pool>] [--worker-runtime modal|local_docker] [--worker-version <version>] [--base-working-root <directory>] [--harness-revision <revision>]"
    ].join("\n")
  );
};
