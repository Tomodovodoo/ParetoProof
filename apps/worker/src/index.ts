...
if (!command) {
  console.error(
    [
      ...
      "  tsx src/index.ts run-problem9-attempt [--help] --benchmark-package-root <directory> --prompt-package-root <directory> --workspace <directory> --output <directory> [--provider-family <family>] [--auth-mode <mode>] [--provider-model <model>] [--model-snapshot-id <id>] [--stub-scenario exact_canonical|compile_failure]",
      "  tsx src/index.ts run-problem9-attempt-in-devbox [--help] --image <docker-image> [--preflight-only] [--print-docker-command] [--benchmark-package-root <directory> --prompt-package-root <directory> --workspace <directory> --output <directory> --provider-model <model>]"
    ].join("\n")
  );
  process.exitCode = 1;
