...
- repository-owned local build commands for the named runtime targets:
  - `bun run build:problem9-execution` builds `problem9-execution` as `local/paretoproof-problem9-execution:local`
  - `bun run build:problem9-devbox` builds `problem9-devbox` as `local/paretoproof-problem9-devbox:local`
  - `bun run build:paretoproof-worker` builds `paretoproof-worker` as `local/paretoproof-worker:local`
  - append `-- --print-only` to any of those commands to emit the canonical Docker command without running it
...
- the canonical local devbox image name for trusted-local validation is `local/paretoproof-problem9-devbox:local`, built via `bun run build:problem9-devbox`
